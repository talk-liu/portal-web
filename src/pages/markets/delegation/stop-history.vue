<template>
  <div class="history-orders-table">
    <div class="thead">
      <ul class="th">
        <li class="max">{{$t('m.exchange.slide')}}</li>
    ...
    <div class="tbody scrollStyle" v-if="historyList.length">
      <ul :class="['tr', item.type]" v-for="item in historyList" :key="item.id">
        ...
        <li>{{item.volume}}</li>
        <li>{{item.total}}</li>
        <li class="right max">{{item.date}}</li>
        <li class="right max">{{item.state==3?$t('m.exchange.cancelled'):formatReason(item.reason)}}</li>
      </ul>
    </div>
